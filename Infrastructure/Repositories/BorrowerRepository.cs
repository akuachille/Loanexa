...

{
    ...
    public BorrowerRepository(IDbContextFactory<ApplicationDbContext> contextFactory, IUserContext userContext)
         ...
         public async Task CreateBorrower(CreateBorrowerDTO borrowerDTO)
        {
            ...
            if (!string.IsNullOrEmpty(borrowerDTO.IdentificationNumber))
            {
                var existingId = await dbContext.Borrowers.AnyAsync(b => b.IdentificationNumber == borrowerDTO.IdentificationNumber);
                if (existingId)
                {
                    throw new ArgumentException("A borrower with this Identification Number already exists.");
                }
            }

            // Uniqueness check for PhoneNumber
            if (!string.IsNullOrEmpty(borrowerDTO.PhoneNumber))
            {
                var existingPhone = await dbContext.Borrowers.AnyAsync(b => b.PhoneNumber == borrowerDTO.PhoneNumber);
                if (existingPhone)
                {
                    throw new ArgumentException("A borrower with this Phone Number already exists.");
                }
            }
    ...
    // 2. Query 'Users' from the dbContext instance, NOT the factory
        ...
        public async Task UpdateBorrower(int Id,UpdateBorrowerDTO borrowerDTO)
        {
            ...
            if (!string.IsNullOrEmpty(borrowerDTO.IdentificationNumber))
            {
                var existingId = await dbContext.Borrowers.AnyAsync(b => b.IdentificationNumber == borrowerDTO.IdentificationNumber && b.Id != Id);
                if (existingId)
                {
                    throw new ArgumentException("A borrower with this Identification Number already exists.");
                }
            ...
            if (!string.IsNullOrEmpty(borrowerDTO.PhoneNumber))
            {
                var existingPhone = await dbContext.Borrowers.AnyAsync(b => b.PhoneNumber == borrowerDTO.PhoneNumber && b.Id != Id);
                if (existingPhone)
                {
                    throw new ArgumentException("A borrower with this Phone Number already exists.");
                }
            ...
            if (_borrower != null)
            {
                ...
                _borrower.IdentificationNumber = borrowerDTO.IdentificationNumber ?? string.Empty;
                _borrower.PhoneNumber = borrowerDTO.PhoneNumber ?? string.Empty;
                _borrower.Email = borrowerDTO.Email ?? string.Empty;
                _borrower.NextOfKin = borrowerDTO.NextOfKin;
                _borrower.KinPhoneNumber = borrowerDTO.KinPhoneNumber;
                _borrower.NextOfKinRelationship = borrowerDTO.NextOfKinRelationship;
                _borrower.Province = borrowerDTO.Province ?? string.Empty;
                _borrower.District = borrowerDTO.District ?? string.Empty;
                _borrower.Sector = borrowerDTO.Sector ?? string.Empty;
                _borrower.Cell = borrowerDTO.Cell ?? string.Empty;
                _borrower.Village = borrowerDTO.Village ?? string.Empty;
